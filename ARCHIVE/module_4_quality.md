...
- [When Quality fails in CCHF diagnosis](#when-quality-fails-in-cchf-diagnosis)
- [The Pre-Analytical Phase](#the-pre-analytical-phase)
  - [Proper collection and labeling](#proper-collection-and-labeling)
- [Assay controls](#assay-controls)
  - [Control monitoring](#control-monitoring)
  - [Trend analysis](#trend-analysis)
  - [Reference material](#reference-material)
- [Proficiency testing](#proficiency-testing)
- [Testing of new reagent lots](#testing-of-new-reagent-lots)
- [Standard Operation procedures](#standard-operation-procedures)
- [Recognize non-conformities and act upon these](#recognize-non-conformities-and-act-upon-these)

## Introduction

In the previous chapter we already discussed some controls and calibrators of the real-time PCR assay. In this chapter we will elaborate more on technical quality indicators of the assay, among others the validation of the test and continues control assessment, for example with proficiency testing.
...
- Setup a technical control system for your assay, containing
  ...
  - Use correct assay controls

  - Interpret trend analysis

  - Use correct reference materials
...
Dr. Ayşe Demir, an infectious disease physician at a regional hospital in eastern Turkey, received a critical call from her laboratory on a Friday afternoon. A 42-year-old livestock farmer had presented to the emergency department with high fever (39.5°C), severe headache and myalgia, and . He reported removing multiple ticks from his body three days earlier while working with sheep.

Given the endemic nature of CCHF in the region and the classic presentation, Dr. Demir immediately ordered a blood platelet count on a whole blood sample and CCHF real-time RT-PCR testing on the patient's serum sample. The laboratory used a commercial RT-PCR kit targeting the S-segment of the CCHFV genome. Within 6 hours the results came back: 
- thrombocytopenia (platelet count: 45,000/µL)
- POSITIVE for CCHF viral RNA with a Ct value of 34.
...
Further investigation revealed a critical quality control failure: On the same day the farmer's sample was processed, the laboratory prepared high viral load CCHF-positive control sample (from a confirmed case with viral load >10^8^ copies/mL) in the same Laminar Flow Cabinet. Cross-contamination during manual extraction (high-load positive control processed before the patient) was the likely culprit---pipette and spatial workflow lapses had not been mitigated. Negative extraction controls had not been included that day.
...
Blank control: he blank should contain the complete reaction mixture *except* nucleic acids (adding used elution buffer). Its use is mandatory to monitor for carry-over contamination (amplification contamination).
...
**Quantitative Control Requirement:** Quantitative procedures must include **two control materials of different concentrations** (calibrators) in each run.
...
https://www.spcforexcel.com/knowledge/measurement-systems-analysis-gage-rr/levey-jennings-charts/
...
Link to this website with overview.
...
Interpreting the Story of the Run
...
**Quantitative Reports:** These provide a numerical value, such as a viral load (e.g., 50,000 copies/mL). These reports must state the test's **analytic measuring range**. A result can be reported as a specific number (if within the range), or as "above" or "below" the range if the quantity is too high or too low to be measured accurately.
